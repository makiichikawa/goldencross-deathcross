module.exports = class Stocks {
  ...
  async getSymbols (industryOrSymbol) {
    const numbers = typeof industryOrSymbol === 'string' ? this.symbolByIndustry[industryOrSymbol] : industryOrSymbol
  ...
  async autoc (num) {
    ...
    try {
      const result = (await yahooFinance2.autoc(num + '.T'))
        .Result
        .filter(element => {
          if ((element.symbol.match(/T$/)) && (element.type === 'S')) {
            return true
          } else {
            return false
          }
